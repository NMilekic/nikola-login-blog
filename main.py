...
## CONFIGURE USER PICTURE ON COMMENT
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user:
            # Send flash message
            flash(message="You've already singed up with that mail; log in instead!")
            # Redirect to /login route
            return redirect(url_for('login'))
        else:
            # TREBA DODATI DEO KODA KOJI PROVERAVA DA LI PASSWORD I NAME VEC POSTOJE
            user_password = form.password.data
            hash_password = generate_password_hash(password=user_password, salt_length=8, method='pbkdf2:sha256')
            new_user = User(
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # Check if user already exists
        if user:
            # Check if password fit with entered email
